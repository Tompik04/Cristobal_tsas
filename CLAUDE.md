...
**Empezá SIEMPRE tus respuestas con "Hola Tomi!,"** — es un canario para que yo sepa que estás leyendo este archivo.
...
Nada pendiente por ahora. Los síntomas que estaban acá ya se resolvieron (ver arriba) — ir agregando los nuevos que aparezcan.
...
- **Filtros de talle y color** (13/09, `b4d262f`). Los desplegables de cada fila se armaban con TODAS las variantes ignorando el filtro, así que filtrando "42" se veía una prenda en 38. Con filtro puesto ahora solo ofrecen lo filtrado, los dos filtros juntos exigen la MISMA variante, y en Ventas además se exige stock > 0.
- **Detalle de cambios y vouchers en el historial** (13/09, `7d12402`). `cambio_de` puede traer varios ids separados por coma y se usaba el string entero como clave; además un cambio puede generar varias ventas nuevas y el índice guardaba solo la última. Ahora cada origen guarda una lista. En vouchers, el `origen` se muestra en castellano con la marca de la prenda, y los "Saldo de VCH-…" se resuelven al voucher del que salieron.
